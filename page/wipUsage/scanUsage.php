...
?>
	...
	<div class="panel panel-default">
		...
		<?php
			...
			if($cekProduk > 0 ){
				//@$param1			= "0407A3L05PRP-STR300grWIPBOX";
				$param1= substr($id, 9, 10);
				//echo $prp_str;
				$cekTgl 				= pg_query($conn,"SELECT * FROM serah_terima WHERE nomor_label ILIKE '%$param1%' AND status=2 ORDER BY tanggal_produksi ASC LIMIT 1");
				$dataTgl = pg_fetch_array($cekTgl);
				$tglParam =  $dataTgl['tanggal_produksi'];
				$tglWip 	=  $fetchProduk['tanggal_produksi'];
				//echo $cek = pg_num_rows($cekTgl);
				if($tglWip == $tglParam){
				?>
					<div class="panel-body" style="">
				<form class="form-horizontal" method="post" action="page/wipUsage/simpanUsage.php" onsubmit="updateStatus()">
					<div class="row">
						<div class="col-md-6" >
				...
				<?php
					...
					<div class="panel-body">
						<div class="alert alert-danger alert-dismissible" role="alert">
						  <strong>Pemakaian tidak FIFO !</strong> Masih ada WIP <?=$param1?> ditanggal <?=$tglParam?> yang belum dipakai, tolong periksa kembali.
