<?php
	...
	/**
	$profile = CapabilityProfile::load("SP2000");
	$connector = new WindowsPrintConnector("smb://".$_SERVER['REMOTE_ADDR']."/wip");
	$printer = new Printer($connector, $profile);
	**/
	$printer -> setJustification(Printer::JUSTIFY_CENTER);
	$printer -> setTextSize(1,1);
	$printer -> text(" \n");
	$printer -> text("REKAP HASIL USAGE WIP\n");
	$printer -> setTextSize(1,1);
	...
	//WIP BOX
	$no = 1;
	$queryWipBox = pg_query($conn,"SELECT * FROM tbl_wip_usage WHERE shift='$_SESSION[shift]' AND regu='$_SESSION[regu]' AND tanggal_usage = '$tgl' AND batch ILIKE '%WIPBOX%'");
	if(pg_num_rows($queryWipBox)){
		...
		while($dataWipBox  = pg_fetch_assoc($queryWipBox)){
			...
			$printer -> text($no++);
			$printer -> setPrintLeftMargin(80);
			if ($dataWipBox['jam'] !== null) {
				$printer->text($dataWipBox['jam']);
			}
			//$printer -> text($dataWipBox['jam']);
			$printer -> setPrintLeftMargin(230);
			$printer -> text($dataWipBox['jumlah_usage']);
			$printer -> text("\n");
			
		}
		$queryTotalWipBox = pg_query($conn, "SELECT SUM(tbl_wip_usage.jumlah_usage) AS total_wip_box, master_produk.isi, master_produk.size FROM tbl_wip_usage JOIN master_produk ON tbl_wip_usage.kode_material = master_produk.kode_material WHERE tbl_wip_usage.shift = '$_SESSION[shift]' AND tbl_wip_usage.regu = '$_SESSION[regu]' AND tbl_wip_usage.tanggal_usage = '$tgl' AND tbl_wip_usage.batch ILIKE '%WIPBOX%' GROUP BY master_produk.isi, master_produk.size");
	...
	// WIP CAR
	$queryWipCar = pg_query($conn,"SELECT * FROM tbl_wip_usage WHERE shift='$_SESSION[shift]' AND regu='$_SESSION[regu]' AND tanggal_usage = '$tgl' AND batch ILIKE '%WIPCAR%'");
	if(pg_num_rows($queryWipCar)){
		...
		while($dataWipCar  = pg_fetch_assoc($queryWipCar)){
			...
			$printer -> setPrintLeftMargin(230);
			$printer -> text($dataWipCar['jumlah_usage']);
			$printer -> text("\n");
			
		}
		$queryTotalWipCar = pg_query($conn, "SELECT SUM(tbl_wip_usage.jumlah_usage) AS total_wip_car, master_produk.isi, master_produk.size FROM tbl_wip_usage JOIN master_produk ON tbl_wip_usage.kode_material = master_produk.kode_material WHERE tbl_wip_usage.shift = '$_SESSION[shift]' AND tbl_wip_usage.regu = '$_SESSION[regu]' AND tbl_wip_usage.tanggal_usage = '$tgl' AND tbl_wip_usage.batch ILIKE '%WIPCAR%' GROUP BY master_produk.isi, master_produk.size");
