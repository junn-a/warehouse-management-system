	...
	<?php
		...
		if(isset($save)){
			$selectProduk 	= pg_query($conn,"SELECT * FROM detil_jadwal WHERE tanggal_jadwal='$tanggal_jadwal' AND kode_material='$kode_material' AND kode_line='$kode_line'");
			$cek 			= pg_num_rows($selectProduk);
			if($cek > 0){
				
			}else{
				$insertData = pg_query($conn,"INSERT INTO detil_jadwal VALUES('$tanggal_jadwal', '$kode_line', '$kode_material', '$jumlah', '00:00:00', '00:00:00', '$keterangan')");
			}
		}
	?>